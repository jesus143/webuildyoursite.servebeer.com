...
class GFAPI {
	...
	public static function update_entry( $entry, $entry_id = null ) {
		...
		$orderStatus         = isset( $entry['orderStatus'] ) ? $entry['orderStatus'] : 'incomplete';
		...
		$result     = $wpdb->query(
			$wpdb->prepare(
                ...
                WHERE
                id = %d
                ", $form_id, $is_starred, $is_read, $ip, $source_url, $user_agent, $currency, $status, $orderStatus, $payment_method, $entry_id
			)
		);
		if ( false === $result ) {
	...
	public static function add_entry( $entry ) {
		...
		$result     = $wpdb->query(
			$wpdb->prepare(
                ...
                (form_id, post_id, date_created, is_starred, is_read, ip, source_url, user_agent, currency, payment_status, payment_date, payment_amount, transaction_id, is_fulfilled, created_by, transaction_type, status, orderStatus, payment_method)
                VALUES
                (%d, {$post_id}, {$date_created}, %d,  %d, %s, %s, %s, %s, {$payment_status}, {$payment_date}, {$payment_amount}, {$transaction_id}, {$is_fulfilled}, {$user_id}, {$transaction_type}, %s, %s, %s)
                ", $form_id, $is_starred, $is_read, $ip, $source_url, $user_agent, $currency, $status, $orderStatus, $payment_method
			)
		);
		if ( false === $result ) {
