...
/**
 * This is to get the form payment status
 * @param $form
 * @return string
 * if null then should return un paid
 * else if approved then should return approved
 */
public static function getPaymentStatus($form)
{
...
public static function getMembershipStatus()
{
	...
	// 	//echo "membership get is not empty <br>";
	// 	$member_tag = $_GET['membership'];
	// } else {
	// 	//echo "membership is empty<br>";
	// }
...
/**
 * Filter what to field value to display ither membership or payment status if approved
 * @param $paymentStatus
 * @param $memberShipStatus
 */
public static function filterPaymentAndMemberShip($paymentStatus, $memberShipStatus)
{
...
//			 echo "payment status $paymentStatus membership status $memberShipStatus field value = $fieldValue <br>";
			...
			if ( is_array( $form['fields'] ) ) {
				...
				foreach ( $form['fields'] as $field ) {
					...
					/**
					 * Hide through member type role
					 * member type
					 * member type is "member" then hide "credit card" and "total" field.
					 */

//					 else if($field->inputName  == 'member_type_value') {
//
...
//                        }
//                        $form_string .= self::get_field($field, 'approved', false, $form, $field_values);
//
//                     }
					 else {
						$field->conditionalLogicFields = self::get_conditional_logic_fields($form, $field->id);
...
//		echo "form id  ". $form_id  . '<br>';
//		echo "form id lead  = " . $form_id_lead . '<br>';
// 	exit;
//
//		global $wpdb;
// 		$results = $wpdb->get_results('SELECT * FROM wp_rg_lead WHERE form_id = ' . $form['id']);
//
//		echo "<pre>";
//			print_r($results);
...
//		echo "force font end <br>";

		//		print_rr($force_frontend_label);
